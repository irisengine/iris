...
struct SimulatedSocket::implementation
{
    int client_queue = -1;
    int server_queue = -1;
    ConcurrentQueue<std::tuple<DataBuffer, std::chrono::steady_clock::time_point>> write_queue;
    std::mutex mutex;
...
{
    impl_->client_queue = ::msgget(std::stoi(client_queue_name), IPC_CREAT | 0666);
    if(impl_->client_queue == -1)
    {
        throw Exception("failed to open client queue");
    }

    impl_->server_queue = ::msgget(std::stoi(server_queue_name), IPC_CREAT | 0666);
    if(impl_->server_queue == -1)
    {
        throw Exception("failed to open server queue");
    }

    // there may be messages in the queue from previous executions (especially if
    ...
    Root::job_system().add_jobs({
        [this]()
        {
            for(;;)
            {
                std::tuple<DataBuffer, std::chrono::steady_clock::time_point> element;

                {
                    ...
                    impl_->signal.wait(lock, [this]() { return !impl_->write_queue.empty(); });
...
std::optional<DataBuffer> SimulatedSocket::try_read(std::size_t count)
{
    ...
    IpcMessage message{ };
    const auto read = ::msgrcv(impl_->client_queue, &message, sizeof(message.mtext), 0, MSG_NOERROR | IPC_NOWAIT);
    if(read == -1)
    {
        if(errno != ENOMSG)
        {
...
DataBuffer SimulatedSocket::read(std::size_t)
{
    // read an IPC message
    IpcMessage message{ };
    const auto read = ::msgrcv(impl_->client_queue, &message, sizeof(message.mtext), 0, MSG_NOERROR);
    if(read == -1)
    {
        throw Exception("read failed");
    }
...
void SimulatedSocket::write(const DataBuffer &buffer)
{
    ...
    std::uniform_int_distribution<> jitter_distribution(-jitter_.count(), jitter_.count());
    ...
    if(!drop_distribution(gen))
    {
        ...
        // notify the send job
        const auto delay = delay_ + std::chrono::milliseconds(jitter_distribution(gen));
        impl_->write_queue.enqueue(buffer, std::chrono::steady_clock::now() + delay);
