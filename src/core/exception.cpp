...
// pointer
template <class T> using auto_free = std::unique_ptr<T, decltype(&std::free)>;
    ...
    , frames_(max_stack_trace_size)
{
    ...
    const auto frame_count =
        ::backtrace(frames_.data(), static_cast<int>(frames_.size()));
    ...
    if (frame_count > 2)
    {
        ...
        frames_ = std::vector<void *>(
            std::cbegin(frames_) + 2u, std::cbegin(frames_) + frame_count - 1u);
    }
}
        ...
        for (auto i = 0u; i < frames_.size(); ++i)
        {
            ...
            if (std::regex_search(symbol, matches, reg))
            {
                ...
                if (demangled && (status == 0))
                {
                    ...
                    const auto second =
                        symbol.substr(split_point + mangled_name.size());
