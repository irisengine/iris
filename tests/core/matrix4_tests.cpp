...
TEST(matrix4, data)
{
    ...
    ASSERT_EQ(std::memcmp(elements.data(), m.data(), sizeof(iris::real) * elements.size()), 0);
}
...
{
    auto m = iris::Matrix4::make_orthographic_projection(100.0f, 100.0f, 100.0f);
    iris::Matrix4 expected{ { { 
...
{
    auto m = iris::Matrix4::make_perspective_projection(0.785398f, 1.0f, 100.0f, 0.1f, 100.0f);
...
{
    auto m = iris::Matrix4::make_look_at(
        { 1.0f, 2.0f, 3.0f },
        { },
        { 0.0f, 1.0f, 0.0f });
